 ...
 * @LastEditTime: 2026-04-01 21:26:16
 * @LastEditors: mulingyuer
 * @Description: 重启 VS Code 窗口状态栏按钮
 * @FilePath: \restart-vscode-server\src\status-bar\restart-vscode-window.ts
 * 怎么可能会有bug！！！
 */
...
import { SHOW_RESTART_VSCODE_WINDOW_BUTTON_SETTING } from "@/constant/status-bar";
import { getExtensionConfiguration } from "@/utils/tools";
import { debounce } from "@/utils/tools";

export class RestartVscodeWindowButton implements StatusBarButton {
	private button: vscode.StatusBarItem;
	/** 设置监听器，与类同生命周期，不会被中途清空 */
	private settingDisposables: vscode.Disposable[] = [];
	private readonly settingKey = `${EXTENSION_NAMESPACE}.${SHOW_RESTART_VSCODE_WINDOW_BUTTON_SETTING}`;

	constructor() {
		this.button = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 0);

		// 绑定要执行的命令
		this.button.command = RESTART_VSCODE_WINDOW_COMMAND;

		// 设置按钮文本和提示
		this.button.text = `${vscode.l10n.t("statusBar.restartVscodeWindow.label")}`;
		this.button.tooltip = vscode.l10n.t("statusBar.restartVscodeWindow.tooltip");
	...
	private async isVisible(): Promise<boolean> {
		const shouldShow = getExtensionConfiguration<boolean>(
			SHOW_RESTART_VSCODE_WINDOW_BUTTON_SETTING,
			true
		);
